...
describe("config execute branching", () => {
  ...
  it("auto-commits config.json when autoVersioning is enabled", async () => {
    ...
    await writeWithBackup(fp, JSON.stringify({ autoVersioning: true }), worktree)
  ...
  it("backups land in root/.opencode/backups not nested", async () => {
    const fp = configPath(worktree)
    await writeWithBackup(fp, JSON.stringify({ autoVersioning: true }), worktree)
    await writeWithBackup(fp, JSON.stringify({ autoVersioning: false }), worktree)
    ...
    const nested = path.join(worktree, ".opencode", "spec-memory", ".opencode", "backups")
    await expect(fs.readdir(nested)).rejects.toThrow()
  })
})
