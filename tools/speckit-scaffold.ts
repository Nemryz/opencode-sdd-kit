...
const MARKER_TEMPLATES = new Set(["spec", "plan", "tasks"])

function getScaffoldContent(template: string, featureName: string): string {
  if (MARKER_TEMPLATES.has(template)) {
    return `# ${template}: ${featureName}\n\n<!-- Content pending skill generation -->\n`
  }
  switch (template) {
    case "data-model":
...
export default tool({
  ...
  async execute(args, context) {
    try {
      ...
      let content = MARKER_TEMPLATES.has(args.template)
        ? getScaffoldContent(args.template, args.featureName)
        : (template ?? getScaffoldContent(args.template, args.featureName))
      content = content
