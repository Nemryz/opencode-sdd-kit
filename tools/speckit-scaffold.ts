...
function getFallbackContent(template: string, featureName: string): string {
  switch (template) {
    case "spec":
    ...
    try {
      ...
      if (args.template === "plan" || args.template === "tasks") {
        ...
        if (existing) {
          ...
          const result = await withLock(specsPath, async () => {
            const num = await getNextFeatureNumber(projectRoot)
            const r = makeFeatureDirName(args.featureName, num)
            const dirName = r.dir
            const fp = path.join(specsPath, dirName)
            await fs.mkdir(fp, { recursive: true })
            return { dirName, num, fp, truncated: r.truncated }
          })
          featureDirName = result.dirName
          featureNumber = result.num
          featurePath = result.fp
          slugTruncated = result.truncated
        }
      } else {
        const result = await withLock(specsPath, async () => {
          const num = await getNextFeatureNumber(projectRoot)
      ...
      let content = template ?? getFallbackContent(args.template, args.featureName)
      content = content
