...
Additionally, read `specs/{metadata.featureDir}/spec.json`. If `approvals.plan.approved` is not `true`, tell the user: the plan needs approval, run `/approve plan`, and stop.
